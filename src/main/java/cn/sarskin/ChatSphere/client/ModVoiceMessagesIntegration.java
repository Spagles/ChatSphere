...
public class ModVoiceMessagesIntegration {
    ...
    private static final Set<UUID> seenUuids = new HashSet<>();
    private static final Queue<VoiceCtx> contextQueue = new ConcurrentLinkedQueue<>();
    ...
    public static void pushVoiceMessageContext(UUID senderUuid, String target) {
        contextQueue.add(new VoiceCtx(senderUuid, target));
    }
    ...
    public static void openRecordingScreen(Screen parent, int btnX, int btnY, String target) {
        ...
        try {
            Minecraft mc = Minecraft.getInstance();
            Object screen = recordScreenCtor.newInstance(parent, btnX, btnY, target);
    ...
    public static void handleVoiceChatMessage(Component message, String tagStr) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null) { LOGGER.info("VM: player null, skip"); return; }
        UUID playbackUuid;
        try {
        ...
        } catch (Exception e) { LOGGER.info("VM: bad uuid from {}", tagStr); return; }
        ...
        LOGGER.info("VM: adding msg name='{}' convId={} type={} isOwn={}", name.getString(), convId, convType, isOwn);
        ChatHistoryManager.getInstance().addMessage(
                name, senderUuid,
                Component.literal("VoiceMessage#" + playbackUuid),
                convId, convType, isOwn);
        LOGGER.info("VM: added successfully");
    }
    ...
    public static void handleIncomingVoice(UUID voiceMessageId, UUID senderUuid, String conversationId,
        ...
        try {
            ...
            if (name == null) name = Component.literal("Unknown");
